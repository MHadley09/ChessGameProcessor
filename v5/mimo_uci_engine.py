#!/usr/bin/env python3
"""
mimo_uci_engine.py — UCI-compatible chess engine powered by MIMO V5 model.

Plays chess like a human of specified Elo rating.  Compatible with CuteChess,
Arena, and any UCI GUI.

V5 features: ContrastiveEncoder, PhaseGatedExperts, AttentionMoveHead —
all automatically loaded from checkpoint config. Phase weights are reported
in UCI info strings.

Usage (standalone):
    python mimo_uci_engine.py \
        --checkpoint checkpoints/v5/best.pt \
        --lc0 path/to/lc0.exe \
...
class MIMOUCIEngine:
    """Full UCI protocol handler wrapping MIMO V5 inference."""

    ENGINE_NAME = "KRUT-MIMO v5"
    ...
    def cmd_go(self, tokens: List[str]):
        ...
        # Move selection (sample from top-K)
        ...
        if 'wdl_before' in preds:
            ...
            # Phase info string (V5)
            phase_str = ""
            if 'phase_weights' in preds:
                pw = preds['phase_weights']
                phase_names = ['open', 'mid', 'end']
                phase_str = " phase=" + "/".join(f"{pw[i]:.0%}" for i in range(len(pw)))

            info_parts.append(
                f"info depth 1 score cp {cp_approx} wdl {w_pm} {d_pm} {l_pm} "
                f"pv {best_move} "
                f"string prob={best_prob:.1%} "
                f"mistake={preds['mistake_prob']:.1%} "
                f"time_pred={preds['predicted_time_s']:.1f}s "
                f"lc0={lc0_ms:.0f}ms mimo={infer_ms:.0f}ms{phase_str}"
            )
        ...
        think_time = max(0.5, predicted_time * noise_factor)
...
def main():
    parser = argparse.ArgumentParser(
        description='KRUT-MIMO v5 — UCI chess engine powered by human-move prediction')
    parser.add_argument('--checkpoint', type=str, default='',
                        help='Path to MIMO V5 checkpoint (.pt)')
